    ...
    const AndroidWebBox = {
        ...
        callNative: function(plugin, action, params, callback) {
            ...
            try {
                ...
                // WebViewJavascriptBridge fallback
                else if (window.WebViewJavascriptBridge) {
                    window.WebViewJavascriptBridge.callHandler('AndroidWebBoxNative', message, function(response) {
                        // Handler if needed
                    });
                }
                else {
                    ...
                    if (callback) {
                        callback({ success: false, error: { code: 'NO_BRIDGE', message: 'No native bridge available' } });
                    }
                }
            } catch (e) {
                console.error('JSBridge call error:', e);
                if (callback) {
                    callback({ success: false, error: { code: 'CALL_ERROR', message: e.toString() } });
                }
            }
